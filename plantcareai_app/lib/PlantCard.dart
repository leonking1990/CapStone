...
class PlantCard extends StatefulWidget {
 ...
 class _PlantCardState extends State<PlantCard>
     with SingleTickerProviderStateMixin { // Need TickerProvider for animation
   ...
   void initState() {
     ...
     _controller = AnimationController(
       vsync: this,
       duration: const Duration(milliseconds: 250), // Short duration for quick press
   ...
   void _handleTapDown(TapDownDetails details) {
     _controller.reverse(from: 1.0); // Animate scale down (reverse from upper bound)
   }

   void _handleTapUp(TapUpDetails details) {
     _controller.forward(from: 0.95).whenComplete(() {
       // This code runs only after the forward animation (scale up) is done
       if (mounted) { // Good practice to check if widget is still mounted
         widget.onTap?.call();
   ...
   Widget build(BuildContext context) {
     ...
     return GestureDetector(
       ...
       child: ScaleTransition( // Apply the scale animation
         scale: _controller, // Use controller directly or _scaleAnimation if using CurvedAnimation
         child: Container( // The original card container
                   ...
                   child: (imageUrl != 'N/A' && imageUrl.isNotEmpty)
                       ? CachedNetworkImage( 
                         ...
                         errorWidget: (context, url, error) => Container(
                           color: placeholderColor,
                           child: Icon(Icons.broken_image, color: secondaryTextColor, size: 40),
                         ),
                       )
                       : Container( /* ... Placeholder setup ... */
                         color: placeholderColor,
                         child: Icon(Icons.local_florist, color: secondaryTextColor, size: 40),
                       ),
                 ),
               ),
               Padding(
