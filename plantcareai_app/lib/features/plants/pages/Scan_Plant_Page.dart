...
class _ScanPlantPageState extends State<ScanPlantPage> {
    ...
    if (args != null) {
      ...
      _plantNameForContext = args['plantName'];
       if (kDebugMode && _isUpdate) {
           print("ScanPage received update context: plantId=$_plantId, name=$_plantNameForContext");
       }
    }
  }
  ...
  Future<void> _pickImageFromGallery() async {
     if (_isAnalyzing) return; // Prevent picking while analyzing
  ...
  Future<void> _captureImageWithCamera() async {
    ...
    if (capturedFile != null && mounted) { // Check mounted after await
      setState(() {
  ...
  Future<void> _analyzePlant() async {
    if (_imageFile == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Center(child: Text('Please select or capture an image first!')),
    ...
    setState(() { _isAnalyzing = true; }); // Show loading indicator
    ...
    try {
       ...
       if (response != null && response['plant_data'] is Map<String, dynamic>) {
         ...
         if (mounted) { // Check mounted before navigation
          ...
          if (mounted) {
             ScaffoldMessenger.of(context).showSnackBar(
               const SnackBar(
                 content: Center(child: Text('Could not analyze plant. Please try again.')),
       ...
       if (mounted) {
         setState(() { _isAnalyzing = false; }); // Hide loading indicator
       }
     }
  }
  ...
  Widget build(BuildContext context) {
     ...
     final String pageTitle = _isUpdate ? 'Re-Scan "$_plantNameForContext"' : 'Scan New Plant';
    ...
    return Scaffold(
      appBar: AppBar(
        title: Row(
          ...
          children: [
            Flexible( // Allow text to wrap or shrink
               child: Text(
        ...
        leading: IconButton(
          ...
          icon: Icon(Icons.arrow_back, color: _isAnalyzing ? Colors.grey : Theme.of(context).appBarTheme.iconTheme?.color),
  ...
  Widget _buildButton({
    ...
    return ElevatedButton.icon(
      ...
      icon: showLoading
          ? Container(
              width: 20, height: 20,
              margin: const EdgeInsets.only(right: 8.0), // Add spacing like icon
      ...
      style: ElevatedButton.styleFrom(
        ...
        foregroundColor: Theme.of(context).colorScheme.onPrimary, // Use theme contrast color
        ...
        elevation: onPressed != null ? 4 : 0, // Reduce elevation when disabled
        textStyle: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600) // Adjust style
      ),
